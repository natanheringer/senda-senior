...
export function ManualSection() {
  ...
  const guias = [
    { num: '01', title: 'PREVENT CARE', color: 'var(--green)', content: 'Construa a fundação de confiança: Diálogos estratégicos, organização de papéis e diretrizes antes de qualquer declínio acontecer.' },
    { num: '02', title: 'CARE', color: 'var(--terracotta)', content: 'A fase de transição: Como manejar perdas leves de autonomia e implementar cuidados sem gerar atrito ou crises.' },
    { num: '03', title: 'IMMEDIATE CARE', color: 'var(--dark)', content: 'Ação na urgência: Suporte prático para tomadas de decisão críticas baseadas no plano estabelecido anteriormente.' },
  ]
    ...
    <section id="manual" style={{
      ...
      <div style={{
            ...
            {guias.map((guia, i) => {
              ...
              return (
                ...
                >
                  ...
                  >
                    <h3 style={{ 
                      fontFamily: 'var(--serif)', fontSize: isHovered ? 'clamp(32px, 3.5vw, 48px)' : 24, 
        ...
        <div>
          <Reveal delay={0.1}>
            <p style={{
              fontSize: 'clamp(20px, 2vw, 24px)', lineHeight: 1.5, color: 'var(--text-sub)',
            ...
            {benefits.map((b, i) => (
              <Reveal key={i} delay={0.25 + i * 0.08}>
                <div style={{
                  ...
                  fontSize: 16, lineHeight: 1.5, color: 'var(--text-sub)', fontWeight: 400,
                }}>
                  <CheckCircle size={18} strokeWidth={2} style={{ color: 'var(--green)', flexShrink: 0, marginTop: 2 }} />
                  {b}
                </div>
              </Reveal>
            ))}
          ...
          <Reveal delay={0.6}>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              <a href="/login" id="manual-buy-btn" style={{
                ...
                onMouseEnter={e => { e.currentTarget.style.background = 'var(--terracotta-dark)'; e.currentTarget.style.transform = 'translateY(-2px)' }}
              ...
              <span style={{ fontSize: 13, color: 'var(--text-muted)', fontStyle: 'italic', paddingLeft: 8 }}>
