...
import { ChevronLeft, Menu, X, Eye, EyeOff, Compass } from 'lucide-react'
import { manualChapters } from '@/lib/manualData'
import Link from 'next/link'

export function DigitalReader({ initialChapterSlug }: { initialChapterSlug: string }) {
  ...
  // Progress Bar vinculada ao scroll do elemento container do texto
  ...
  return (
    <div style={{ display: 'flex', height: '100vh', width: '100vw', overflow: 'hidden', background: 'var(--cream)' }}>
      ...
      {/* ─── SIDEBAR GLASSSMORPHISM ─── */}
      <motion.aside
        initial={false}
        animate={{ width: sidebarOpen ? 340 : 0, opacity: sidebarOpen ? 1 : 0 }}
        style={{
           overflow: 'hidden', zIndex: 50, borderRight: '1px solid rgba(0,0,0,0.05)',
           background: 'rgba(254,252,249,0.7)', backdropFilter: 'blur(30px)' // Glassmorphism pesado
        }}
      >
         <div style={{ padding: 32, width: 340, height: '100%', display: 'flex', flexDirection: 'column' }}>
            <Link href="/" style={{ display: 'flex', alignItems: 'center', gap: 12, textDecoration: 'none', color: 'var(--green)', marginBottom: 48, flexShrink: 0 }}>
               <Compass size={24} strokeWidth={1.5} />
               <span style={{ fontFamily: 'var(--serif)', fontWeight: 600, fontSize: 18 }}>Senda Sênior</span>
            </Link>
            
            <p className="label-premium" style={{ color: 'var(--text-muted)', marginBottom: 24, flexShrink: 0 }}>Índice Temático</p>
                 ...
                 <Link key={ch.slug} href={`/manual/${ch.slug}`} style={{ textDecoration: 'none' }}>
                   <motion.div 
                     style={{ 
                       ...
                       background: ch.slug === chapter.slug ? 'var(--green)' : 'transparent',
                       color: ch.slug === chapter.slug ? 'white' : 'var(--text-sub)',
                     ...
                     whileHover={{ 
                       background: ch.slug === chapter.slug ? 'var(--green)' : 'rgba(45,95,79,0.03)',
      ...
      <div style={{ flex: 1, position: 'relative', display: 'flex', flexDirection: 'column', height: '100vh', opacity: focusMode && sidebarOpen ? 0.3 : 1, transition: 'opacity 0.4s' }}>
        ...
        <motion.header
          ...
          style={{
             ...
             background: 'linear-gradient(to bottom, var(--cream) 10%, transparent)', pointerEvents: focusMode ? 'none' : 'auto'
          }}
        >
           <button 
             onClick={() => setSidebarOpen(!sidebarOpen)}
             style={{ display: 'flex', alignItems: 'center', gap: 12, background: 'white', border: '1px solid rgba(0,0,0,0.05)', padding: '12px 24px', borderRadius: 30, cursor: 'pointer', boxShadow: '0 4px 16px rgba(0,0,0,0.04)', color: 'var(--text)', fontWeight: 600, transition: 'all 0.2s' }}
           >
           ...
           <button 
             ...
             style={{ display: 'flex', alignItems: 'center', gap: 12, background: 'var(--green)', color: 'white', border: 'none', padding: '12px 24px', borderRadius: 30, cursor: 'pointer', boxShadow: '0 8px 24px rgba(45,95,79,0.2)', fontWeight: 600, transition: 'all 0.2s' }}
           >
              <EyeOff size={18} />
              Modo Foco
           </button>
        ...
        {/* Progress Bar do Manual */}
        <motion.div 
           style={{ position: 'fixed', top: 0, left: sidebarOpen ? 340 : 0, right: 0, height: 4, background: 'var(--terracotta)', scaleX, originX: 0, zIndex: 100, transition: 'left 0.3s' }} 
        />
        ...
        <div ref={containerRef} style={{ flex: 1, overflowY: 'auto', padding: '160px clamp(20px, 4vw, 40px) 120px', scrollBehavior: 'smooth' }}>
           ...
           <AnimatePresence>
             {focusMode && (
                <motion.button
                  ...
                  style={{ position: 'fixed', bottom: 40, right: 40, background: 'white', color: 'var(--green)', border: '1px solid rgba(0,0,0,0.1)', padding: '16px', borderRadius: '50%', cursor: 'pointer', boxShadow: '0 16px 48px rgba(0,0,0,0.1)', zIndex: 60 }}
                >
                   <Eye size={24} />
                </motion.button>
             )}
           ...
           <article style={{ maxWidth: 740, margin: '0 auto', opacity: sidebarOpen && typeof window !== 'undefined' && window.innerWidth < 1000 ? 0.3 : 1, transition: 'opacity 0.3s' }} onClick={() => sidebarOpen && window.innerWidth < 1000 && setSidebarOpen(false)}>
              <motion.div initial={{ opacity: 0, y: 40 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, ease: [0.25, 0.46, 0.45, 0.94] }}>
                ...
                <h1 style={{ fontFamily: 'var(--serif)', fontSize: 'clamp(36px, 5vw, 64px)', fontWeight: 500, color: 'var(--text)', lineHeight: 1.05, marginBottom: 0, letterSpacing: '-0.02em' }}>{chapter.title}</h1>
                   ...
                   {chapter.content.map((paragraph, idx) => {
                     ...
                     if (isHighlight) {
                       return (
                         <motion.h2 
                            ...
                            style={{ fontFamily: 'var(--serif)', fontSize: 32, color: 'var(--green)', fontWeight: 600, marginTop: 48, paddingBottom: 16, borderBottom: '1.5px solid rgba(45,95,79,0.15)' }}
                         >
                            {paragraph.replace('### ', '')}
                         </motion.h2>
                       )
                     ...
                     return (
                       <motion.p
                         ...
                         style={{ 
                            fontSize: 21, lineHeight: 1.85, color: isImportant ? 'var(--text)' : 'rgba(42,37,41,0.88)', 
              ...
              <div style={{ marginTop: 120, paddingTop: 64, borderTop: '1px solid rgba(0,0,0,0.08)', display: 'flex', justifyContent: 'space-between', gap: 24, flexDirection: 'row' }}>
                 {currentChapterIndex > 0 ? (
                    <Link href={`/manual/${manualChapters[currentChapterIndex - 1].slug}`} style={{ textDecoration: 'none', display: 'flex', flexDirection: 'column', gap: 12, flex: 1, padding: 32, background: 'rgba(0,0,0,0.02)', borderRadius: 12 }}>
                       <span className="label-premium" style={{ color: 'var(--text-muted)' }}>Capítulo Anterior</span>
                       <span style={{ fontFamily: 'var(--serif)', fontSize: 24, color: 'var(--text)', fontWeight: 500 }}>{manualChapters[currentChapterIndex - 1].title}</span>
                    </Link>
                 ) : <div style={{ flex: 1 }} />}
                 
                 {currentChapterIndex < manualChapters.length - 1 ? (
                    <Link href={`/manual/${manualChapters[currentChapterIndex + 1].slug}`} style={{ textDecoration: 'none', display: 'flex', flexDirection: 'column', gap: 12, flex: 1, padding: 32, background: 'var(--green)', borderRadius: 12, textAlign: 'right' }}>
                       <span className="label-premium" style={{ color: 'rgba(255,255,255,0.6)' }}>Próxima Etapa</span>
                       <span style={{ fontFamily: 'var(--serif)', fontSize: 24, color: 'white', fontWeight: 500 }}>{manualChapters[currentChapterIndex + 1].title}</span>
                    </Link>
                 ) : <div style={{ flex: 1 }} />}
              </div>
           </article>
